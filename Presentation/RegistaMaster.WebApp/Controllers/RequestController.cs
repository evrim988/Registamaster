...
public class RequestController : Controller
{
   ...
   public async Task<IActionResult> AddActionItem([FromBody] Action model, int ID)
   {
      try
      {
         ...
         {
            var cancelledActions = uow.Repository.GetQueryable<Action>(t => t.RequestID == ID && t.Status == Status.Active && t.ObjectStatus == ObjectStatus.NonDeleted && t.ActionStatus == ActionStatus.Cancel);
            foreach (var action in cancelledActions)
            {
               action.Status = Status.Passive;
               uow.Repository.Update<Action>(action);
            }
